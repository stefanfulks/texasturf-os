...
- **Standing authorization — additive migrations (safe autonomy):** Non-destructive DDL — `CREATE TABLE`, `ADD COLUMN`, `CREATE INDEX`, new RLS policies, new functions/triggers, additive enum values — may be applied autonomously via `supabase db push` as part of normal feature work. A general "ship it" / "deploy it" instruction **is** sufficient — no separate per-change approval needed. Flow: apply → `pnpm typegen` → `pnpm typecheck` → report. This is the default path for schema changes.
- **Destructive DDL is the exception — explicit, per-action approval every time** — `DROP TABLE`, `DROP COLUMN`, `ALTER COLUMN ... TYPE`, `RENAME COLUMN/TABLE`, `TRUNCATE`, removing or renaming enum values. Stop, surface exactly what data is at risk, and offer a safer path (additive migration + backfill + later cleanup) first. A general "deploy it all" does **not** authorize these.
