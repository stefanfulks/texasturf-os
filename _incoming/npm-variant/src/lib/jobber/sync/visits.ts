...
export async function syncVisitsInRange(
  ...
  let total = 0;
  do {
    const resp: { visits: { pageInfo: { hasNextPage: boolean; endCursor: string | null }; nodes: GqlVisit[] } } =
...
export async function syncVisit(accountId: string, visitId: string) {
  const client = await jobberClient(accountId);
  const resp = await client.request<{ visit: GqlVisit | null }>(ONE_VISIT, { id: visitId });
