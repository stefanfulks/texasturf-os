  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setOpen(false)} className="text-zinc-400 hover:text-zinc-700 text-sm">✕</button>
      </div>
      ...
      <form action={formAction} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {(freq === "weekly" || freq === "biweekly") && (
            <div>
              <label className="block text-xs font-medium text-zinc-500 mb-1">Day of Week</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-500 mb-1">Lead days</label>
            <input type="number" name="lead_days" defaultValue={0} min={0} max={30} className={field} />
            <p className="text-[11px] text-zinc-400 mt-1">Create task N days before due</p>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2">
          <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 text-sm text-zinc-600 hover:text-zinc-900">Cancel</button>
          <button
            type="submit"
            disabled={isPending}
