...
export function InvoiceCommentSection({
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
      ...
      {/* Comments list */}
      {comments.length === 0 ? (
        <div className="px-5 py-6 text-sm text-zinc-400">No comments yet.</div>
      ) : (
            ...
            <div key={c.id} className="px-5 py-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-400">{format(parseISO(c.created_at), "MMM d, h:mm a")}</span>
                </div>
              </div>
      ...
      {/* Add comment */}
      <form action={formAction} className="border-t border-zinc-100 p-5 space-y-3">
        <input type="hidden" name="invoice_id" value={invoiceId} />
        <textarea
          name="body"
          rows={3}
          placeholder="Add a comment…"
          className="w-full text-sm border border-zinc-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-zinc-400 resize-none"
          required
        />
        {isOfficeOrAdmin && (
          <div className="flex items-center gap-2">
            <select name="visibility" defaultValue="internal" className="text-xs border border-zinc-200 rounded px-2 py-1 focus:outline-none">
