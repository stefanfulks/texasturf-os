  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm pt-2 border-t border-zinc-100">
          {invoice.service_period_start && (
            <div className="flex gap-2"><span className="text-zinc-400">Service</span><span>{format(parseISO(invoice.service_period_start), "MMM d")}{invoice.service_period_end ? `–${format(parseISO(invoice.service_period_end), "MMM d")}` : ""}</span></div>
          )}
          {invoice.customer_name && <div className="flex gap-2"><span className="text-zinc-400">Customer</span><span>{invoice.customer_name}</span></div>}
          {invoice.job_name && <div className="flex gap-2"><span className="text-zinc-400">Job</span><span>{invoice.job_name}</span></div>}
          {invoice.approval_deadline && <div className="flex gap-2"><span className="text-zinc-400">Deadline</span><span>{format(parseISO(invoice.approval_deadline), "MMM d, yyyy")}</span></div>}
        </div>

        {invoice.admin_notes && (
          <div className="pt-2 border-t border-zinc-100">
            <p className="text-xs font-semibold text-zinc-400 mb-1">Admin Notes</p>
            <p className="text-sm text-zinc-700">{invoice.admin_notes}</p>
          </div>
        )}
        {invoice.variance_notes && (
          <div className="pt-2 border-t border-zinc-100">
            <p className="text-xs font-semibold text-amber-600 mb-1">⚠ Variance Notes</p>
            <p className="text-sm text-zinc-700">{invoice.variance_notes}</p>
          </div>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tfoot>
              <tr className="bg-zinc-50">
                <td className="px-5 py-3 font-semibold text-zinc-600">Total</td>
