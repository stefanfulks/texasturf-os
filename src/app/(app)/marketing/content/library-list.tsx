      ...
      {SERVICE_LINES.map((s) => (
        <button
          ...
          className={`text-[11px] px-2 py-0.5 rounded-full border ${
            item.service_line === s ? "border-brand-strong bg-brand text-on-brand" : "border-line text-ink-3 hover:bg-hover"
          }`}
        >
          {s.replace(/_/g, " ")}
        </button>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        <Link href={qs({ type: null })} className={`text-xs px-2.5 py-1 rounded-full border ${!activeType ? "border-brand-strong bg-brand text-on-brand" : "border-line text-ink-2 hover:bg-hover"}`}>
          All types
        </Link>
        {TYPE_FILTERS.map((t) => (
          <Link key={t} href={qs({ type: t })} className={`text-xs px-2.5 py-1 rounded-full border ${activeType === t ? "border-brand-strong bg-brand text-on-brand" : "border-line text-ink-2 hover:bg-hover"}`}>
            {TYPE_LABEL[t]}
          </Link>
        ))}
      </div>
      {serviceLines.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          <Link href={qs({ service: null })} className={`text-xs px-2.5 py-1 rounded-full border ${!activeService ? "border-brand-strong bg-brand text-on-brand" : "border-line text-ink-2 hover:bg-hover"}`}>
            All services
          </Link>
          {serviceLines.map((s) => (
            <Link key={s} href={qs({ service: s })} className={`text-xs px-2.5 py-1 rounded-full border ${activeService === s ? "border-brand-strong bg-brand text-on-brand" : "border-line text-ink-2 hover:bg-hover"}`}>
              {s.replace(/_/g, " ")}
            </Link>
          ))}
        </div>
