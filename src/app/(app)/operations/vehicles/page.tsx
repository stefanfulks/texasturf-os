    ...
    <div className="space-y-6">
      ...
      {logs.length === 0 ? (
        <div className="rounded-xl border border-dashed border-zinc-200 bg-white p-10 text-center">
          <p className="text-sm font-medium text-zinc-700">No maintenance logged yet for this filter.</p>
              ...
              {logs.map((r) => (
                <tr key={r.id} className="border-t border-zinc-200 hover:bg-zinc-50">
                  ...
                  <td className="px-3 py-2">
                    <div className="font-medium text-zinc-900">{r.asset?.name ?? "—"}</div>
                    <div className="text-xs text-zinc-500 capitalize">{r.asset?.unit_type.replace("_", " ")}</div>
                  </td>
                  <td className="px-3 py-2 text-zinc-700 max-w-md truncate">{r.description}</td>
